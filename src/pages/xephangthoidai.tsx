            ...
            {champions.map((champ, idx) => (
              <div
                ...
                className={`relative overflow-hidden rounded-2xl border transition-all duration-300 hover:shadow-2xl card-hover-fx group flex flex-col justify-between ${
                  champ.isLatest
                    ? "neon-ring-pulse bg-gradient-to-b from-amber-50/50 via-white to-slate-50 border-amber-400 shadow-lg ring-2 ring-amber-400/40"
              ...
              >
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div className="space-y-2 pt-1">
                    <div className="flex items-center justify-between">
                      <h3 className="font-fco font-black text-xl sm:text-2xl uppercase tracking-tight text-slate-900 group-hover:text-amber-700 transition-colors">
                        {champ.name}
                      </h3>
                      <span className="text-xs font-fco font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200">
                        {champ.stats.matches}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 font-medium">
                      Danh hiệu: <strong className="text-slate-800">{champ.stats.record}</strong>
                    </p>
                  </div>
                </div>

                <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                  <span>Vinh danh tại Sao Vàng Cup</span>
